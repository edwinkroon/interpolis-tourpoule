...
-- Step 1: Clear fantasy_cumulative_points (depends on fantasy_stage_points)
TRUNCATE TABLE fantasy_cumulative_points RESTART IDENTITY;

-- Step 2: Clear fantasy_stage_points (depends on stage_results)
TRUNCATE TABLE fantasy_stage_points RESTART IDENTITY;

-- Step 3: Clear stage_results
TRUNCATE TABLE stage_results RESTART IDENTITY;

-- Step 4: Clear stage_jersey_wearers (optional, but recommended for clean start)
TRUNCATE TABLE stage_jersey_wearers RESTART IDENTITY;

-- Verify the tables are empty
SELECT 'fantasy_cumulative_points' as table_name, COUNT(*) as remaining_rows FROM fantasy_cumulative_points
UNION ALL
SELECT 'fantasy_stage_points', COUNT(*) FROM fantasy_stage_points
UNION ALL
...
SELECT setval('fantasy_cumulative_points_id_seq', 1, false);
