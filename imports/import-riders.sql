-- SQL Script to import riders from riders.csv into riders table
-- This file is auto-generated from riders.csv
-- Run this script in your SQL editor
-- Make sure teams_pro table is already populated before running this script

-- First, ensure the riders table exists with the correct structure
CREATE TABLE IF NOT EXISTS riders (
  id SERIAL PRIMARY KEY,
  team_pro_id INTEGER,
  first_name VARCHAR(100),
  last_name VARCHAR(100) NOT NULL,
  date_of_birth DATE,
  nationality VARCHAR(80),
  weight_kg NUMERIC(4, 1),
  height_m NUMERIC(3, 2),
  photo_url TEXT
);

-- Add foreign key constraint if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'riders_team_pro_id_fkey'
  ) THEN
    ALTER TABLE riders 
    ADD CONSTRAINT riders_team_pro_id_fkey 
    FOREIGN KEY (team_pro_id) REFERENCES teams_pro(id);
  END IF;
END $$;

-- Clear existing riders data before importing
TRUNCATE TABLE riders RESTART IDENTITY CASCADE;

-- Insert riders from riders.csv with all fields
-- Using subquery to match team_name with teams_pro to get team_pro_id
INSERT INTO riders (team_pro_id, first_name, last_name, date_of_birth, nationality, weight_kg, height_m, photo_url)
SELECT 
  tp.id as team_pro_id,
  r.first_name,
  r.last_name,
  r.date_of_birth::DATE,
  r.nationality,
  r.weight_kg::NUMERIC(4,1),
  r.height_m::NUMERIC(3,2),
  NULLIF(r.photo_url, '') as photo_url
FROM (
  VALUES
    ('Tadej', 'Pogačar', 'UAE Team Emirates', 'Slovenia', '1998-09-21', 66.0, 1.76, ''),
    ('João', 'Almeida', 'UAE Team Emirates', 'Portugal', '1998-08-05', 60.0, 1.78, ''),
    ('Jhonatan', 'Narváez', 'UAE Team Emirates', 'Ecuador', '1997-06-04', 65.0, 1.80, ''),
    ('Nils', 'Politt', 'UAE Team Emirates', 'Germany', '1994-03-06', 75.0, 1.85, ''),
    ('Pavel', 'Sivakov', 'UAE Team Emirates', 'France', '1997-07-11', 65.0, 1.88, ''),
    ('Marc', 'Soler', 'UAE Team Emirates', 'Spain', '1993-11-22', 70.0, 1.86, ''),
    ('Tim', 'Wellens', 'UAE Team Emirates', 'Belgium', '1991-05-10', 68.0, 1.84, ''),
    ('Adam', 'Yates', 'UAE Team Emirates', 'United Kingdom', '1992-08-07', 58.0, 1.73, ''),
    ('Jonas', 'Vingegaard', 'Team Visma-Lease a Bike', 'Denmark', '1996-12-10', 60.0, 1.75, ''),
    ('Wout', 'van Aert', 'Team Visma-Lease a Bike', 'Belgium', '1994-09-15', 78.0, 1.90, ''),
    ('Sepp', 'Kuss', 'Team Visma-Lease a Bike', 'United States', '1994-09-13', 61.0, 1.80, ''),
    ('Christophe', 'Laporte', 'Team Visma-Lease a Bike', 'France', '1992-12-11', 72.0, 1.88, ''),
    ('Dylan', 'van Baarle', 'Team Visma-Lease a Bike', 'Netherlands', '1992-05-21', 75.0, 1.90, ''),
    ('Tiesj', 'Benoot', 'Team Visma-Lease a Bike', 'Belgium', '1994-03-11', 70.0, 1.90, ''),
    ('Edoardo', 'Affini', 'Team Visma-Lease a Bike', 'Italy', '1996-06-24', 75.0, 1.90, ''),
    ('Victor', 'Campenaerts', 'Team Visma-Lease a Bike', 'Belgium', '1991-10-28', 75.0, 1.88, ''),
    ('Remco', 'Evenepoel', 'Soudal Quick-Step', 'Belgium', '2000-01-25', 61.0, 1.71, ''),
    ('Julian', 'Alaphilippe', 'Soudal Quick-Step', 'France', '1992-06-11', 62.0, 1.73, ''),
    ('Yves', 'Lampaert', 'Soudal Quick-Step', 'Belgium', '1991-04-10', 78.0, 1.88, ''),
    ('Kasper', 'Asgreen', 'Soudal Quick-Step', 'Denmark', '1995-02-08', 75.0, 1.92, ''),
    ('Mikkel', 'Honoré', 'Soudal Quick-Step', 'Denmark', '1997-01-21', 68.0, 1.85, ''),
    ('Mattia', 'Cattaneo', 'Soudal Quick-Step', 'Italy', '1990-10-25', 68.0, 1.80, ''),
    ('Pascal', 'Eenkhoorn', 'Soudal Quick-Step', 'Netherlands', '1997-02-08', 72.0, 1.88, ''),
    ('Tim', 'Merlier', 'Soudal Quick-Step', 'Belgium', '1992-10-30', 78.0, 1.85, ''),
    ('Primož', 'Roglič', 'Bora-Hansgrohe', 'Slovenia', '1989-10-29', 65.0, 1.77, ''),
    ('Jai', 'Hindley', 'Bora-Hansgrohe', 'Australia', '1996-05-05', 58.0, 1.75, ''),
    ('Emanuel', 'Buchmann', 'Bora-Hansgrohe', 'Germany', '1992-11-18', 60.0, 1.75, ''),
    ('Aleksandr', 'Vlasov', 'Bora-Hansgrohe', 'Russia', '1996-04-23', 65.0, 1.85, ''),
    ('Danny', 'van Poppel', 'Bora-Hansgrohe', 'Netherlands', '1993-07-26', 78.0, 1.85, ''),
    ('Matteo', 'Sobrero', 'Bora-Hansgrohe', 'Italy', '1997-05-14', 68.0, 1.85, ''),
    ('Nico', 'Denz', 'Bora-Hansgrohe', 'Germany', '1994-02-15', 70.0, 1.85, ''),
    ('Marco', 'Haller', 'Bora-Hansgrohe', 'Austria', '1991-04-01', 78.0, 1.88, ''),
    ('Geraint', 'Thomas', 'INEOS Grenadiers', 'United Kingdom', '1986-05-25', 71.0, 1.83, ''),
    ('Tom', 'Pidcock', 'INEOS Grenadiers', 'United Kingdom', '1999-07-30', 58.0, 1.70, ''),
    ('Carlos', 'Rodríguez', 'INEOS Grenadiers', 'Spain', '2001-02-02', 58.0, 1.73, ''),
    ('Egan', 'Bernal', 'INEOS Grenadiers', 'Colombia', '1997-01-13', 60.0, 1.75, ''),
    ('Filippo', 'Ganna', 'INEOS Grenadiers', 'Italy', '1996-07-25', 82.0, 1.93, ''),
    ('Laurens', 'De Plus', 'INEOS Grenadiers', 'Belgium', '1995-09-04', 68.0, 1.85, ''),
    ('Omar', 'Fraile', 'INEOS Grenadiers', 'Spain', '1990-07-17', 68.0, 1.83, ''),
    ('Michal', 'Kwiatkowski', 'INEOS Grenadiers', 'Poland', '1990-06-02', 70.0, 1.76, ''),
    ('Magnus', 'Cort', 'EF Education-EasyPost', 'Denmark', '1993-01-16', 72.0, 1.85, ''),
    ('Neilson', 'Powless', 'EF Education-EasyPost', 'United States', '1996-09-03', 70.0, 1.88, ''),
    ('Rigoberto', 'Urán', 'EF Education-EasyPost', 'Colombia', '1987-01-26', 62.0, 1.73, ''),
    ('Richard', 'Carapaz', 'EF Education-EasyPost', 'Ecuador', '1993-05-23', 62.0, 1.70, ''),
    ('Ben', 'Healy', 'EF Education-EasyPost', 'Ireland', '2000-09-11', 65.0, 1.80, ''),
    ('Vincenzo', 'Albanese', 'EF Education-EasyPost', 'Italy', '1996-11-12', 68.0, 1.80, ''),
    ('Alex', 'Baudin', 'EF Education-EasyPost', 'France', '1996-05-25', 68.0, 1.85, ''),
    ('Harrison', 'Sweeny', 'EF Education-EasyPost', 'Australia', '1997-09-12', 68.0, 1.85, ''),
    ('Thibaut', 'Pinot', 'Groupama-FDJ', 'France', '1990-05-29', 60.0, 1.80, ''),
    ('David', 'Gaudu', 'Groupama-FDJ', 'France', '1996-10-10', 62.0, 1.73, ''),
    ('Valentin', 'Madouas', 'Groupama-FDJ', 'France', '1996-07-12', 68.0, 1.85, ''),
    ('Stefan', 'Küng', 'Groupama-FDJ', 'Switzerland', '1993-11-16', 78.0, 1.94, ''),
    ('Romain', 'Grégoire', 'Groupama-FDJ', 'France', '2003-03-24', 65.0, 1.80, ''),
    ('Quentin', 'Pacher', 'Groupama-FDJ', 'France', '1992-01-06', 68.0, 1.85, ''),
    ('Lars', 'van den Berg', 'Groupama-FDJ', 'Netherlands', '1998-07-07', 70.0, 1.88, ''),
    ('Michael', 'Storer', 'Groupama-FDJ', 'Australia', '1997-02-28', 65.0, 1.80, ''),
    ('Pello', 'Bilbao', 'Bahrain Victorious', 'Spain', '1990-02-25', 65.0, 1.78, ''),
    ('Wout', 'Poels', 'Bahrain Victorious', 'Netherlands', '1987-10-01', 70.0, 1.85, ''),
    ('Matej', 'Mohorič', 'Bahrain Victorious', 'Slovenia', '1994-10-19', 70.0, 1.85, ''),
    ('Santiago', 'Buitrago', 'Bahrain Victorious', 'Colombia', '1999-09-11', 58.0, 1.70, ''),
    ('Dylan', 'Teuns', 'Bahrain Victorious', 'Belgium', '1992-03-01', 70.0, 1.85, ''),
    ('Phil', 'Bauhaus', 'Bahrain Victorious', 'Germany', '1994-07-08', 78.0, 1.88, ''),
    ('Kamil', 'Gradek', 'Bahrain Victorious', 'Poland', '1990-09-17', 75.0, 1.88, ''),
    ('Jack', 'Haig', 'Bahrain Victorious', 'Australia', '1993-09-06', 62.0, 1.78, ''),
    ('Mathieu', 'van der Poel', 'Alpecin-Deceuninck', 'Netherlands', '1995-01-19', 75.0, 1.84, ''),
    ('Jasper', 'Philipsen', 'Alpecin-Deceuninck', 'Belgium', '1998-03-02', 78.0, 1.85, ''),
    ('Søren', 'Kragh Andersen', 'Alpecin-Deceuninck', 'Denmark', '1994-08-10', 72.0, 1.88, ''),
    ('Silvan', 'Dillier', 'Alpecin-Deceuninck', 'Switzerland', '1990-08-03', 72.0, 1.88, ''),
    ('Jonas', 'Rickaert', 'Alpecin-Deceuninck', 'Belgium', '1994-02-07', 78.0, 1.88, ''),
    ('Gianni', 'Vermeersch', 'Alpecin-Deceuninck', 'Belgium', '1992-11-19', 75.0, 1.88, ''),
    ('Ramon', 'Sinkeldam', 'Alpecin-Deceuninck', 'Netherlands', '1989-02-09', 78.0, 1.90, ''),
    ('Jasper', 'De Buyst', 'Alpecin-Deceuninck', 'Belgium', '1993-11-24', 75.0, 1.88, ''),
    ('Enric', 'Mas', 'Movistar Team', 'Spain', '1995-01-07', 60.0, 1.77, ''),
    ('Alejandro', 'Valverde', 'Movistar Team', 'Spain', '1980-04-25', 61.0, 1.78, ''),
    ('Iván', 'García Cortina', 'Movistar Team', 'Spain', '1995-11-20', 70.0, 1.85, ''),
    ('Nelson', 'Oliveira', 'Movistar Team', 'Portugal', '1989-03-06', 68.0, 1.80, ''),
    ('Carlos', 'Verona', 'Movistar Team', 'Spain', '1992-11-04', 62.0, 1.78, ''),
    ('Imanol', 'Erviti', 'Movistar Team', 'Spain', '1983-11-15', 72.0, 1.88, ''),
    ('Jorge', 'Arcas', 'Movistar Team', 'Spain', '1992-07-08', 68.0, 1.85, ''),
    ('Oier', 'Lazkano', 'Movistar Team', 'Spain', '1999-11-07', 70.0, 1.85, ''),
    ('Peter', 'Sagan', 'Lidl-Trek', 'Slovakia', '1990-01-26', 78.0, 1.84, ''),
    ('Giulio', 'Ciccone', 'Lidl-Trek', 'Italy', '1994-12-20', 58.0, 1.75, ''),
    ('Bauke', 'Mollema', 'Lidl-Trek', 'Netherlands', '1986-11-26', 68.0, 1.83, ''),
    ('Jasper', 'Stuyven', 'Lidl-Trek', 'Belgium', '1992-04-17', 75.0, 1.88, ''),
    ('Mads', 'Pedersen', 'Lidl-Trek', 'Denmark', '1995-12-18', 78.0, 1.85, ''),
    ('Quinn', 'Simmons', 'Lidl-Trek', 'United States', '2001-05-08', 65.0, 1.80, ''),
    ('Toms', 'Skujiņš', 'Lidl-Trek', 'Latvia', '1991-06-15', 70.0, 1.85, ''),
    ('Edward', 'Theuns', 'Lidl-Trek', 'Belgium', '1991-04-30', 75.0, 1.88, ''),
    ('Michael', 'Woods', 'Israel-Premier Tech', 'Canada', '1986-10-12', 68.0, 1.83, ''),
    ('Chris', 'Froome', 'Israel-Premier Tech', 'United Kingdom', '1985-05-20', 69.0, 1.86, ''),
    ('Jakob', 'Fuglsang', 'Israel-Premier Tech', 'Denmark', '1985-03-22', 68.0, 1.82, ''),
    ('Guillaume', 'Boivin', 'Israel-Premier Tech', 'Canada', '1989-05-25', 78.0, 1.88, ''),
    ('Derek', 'Gee', 'Israel-Premier Tech', 'Canada', '1997-08-04', 68.0, 1.85, ''),
    ('Hugo', 'Houle', 'Israel-Premier Tech', 'Canada', '1990-09-27', 70.0, 1.85, ''),
    ('Krists', 'Neilands', 'Israel-Premier Tech', 'Latvia', '1994-08-18', 70.0, 1.85, ''),
    ('Nick', 'Schultz', 'Israel-Premier Tech', 'Australia', '1994-09-13', 68.0, 1.83, ''),
    ('Simon', 'Yates', 'Team Jayco AlUla', 'United Kingdom', '1992-08-07', 58.0, 1.72, ''),
    ('Michael', 'Matthews', 'Team Jayco AlUla', 'Australia', '1990-09-26', 72.0, 1.81, ''),
    ('Dylan', 'Groenewegen', 'Team Jayco AlUla', 'Netherlands', '1993-06-21', 78.0, 1.88, ''),
    ('Luke', 'Plapp', 'Team Jayco AlUla', 'Australia', '2000-12-25', 68.0, 1.85, ''),
    ('Eddie', 'Dunbar', 'Team Jayco AlUla', 'Ireland', '1996-09-01', 62.0, 1.78, ''),
    ('Lawson', 'Craddock', 'Team Jayco AlUla', 'United States', '1992-02-20', 70.0, 1.85, ''),
    ('Chris', 'Harper', 'Team Jayco AlUla', 'Australia', '1994-11-23', 68.0, 1.85, ''),
    ('Luka', 'Mezgec', 'Team Jayco AlUla', 'Slovenia', '1988-06-27', 72.0, 1.88, ''),
    ('Arnaud', 'Démare', 'Arkéa-B&B Hotels', 'France', '1991-08-26', 78.0, 1.81, ''),
    ('Warren', 'Barguil', 'Arkéa-B&B Hotels', 'France', '1991-10-28', 65.0, 1.80, ''),
    ('Nacer', 'Bouhanni', 'Arkéa-B&B Hotels', 'France', '1990-07-25', 72.0, 1.78, ''),
    ('Bryan', 'Coquard', 'Arkéa-B&B Hotels', 'France', '1992-04-25', 68.0, 1.70, ''),
    ('Amaury', 'Capiot', 'Arkéa-B&B Hotels', 'Belgium', '1993-06-25', 75.0, 1.88, ''),
    ('Donavan', 'Grinond', 'Arkéa-B&B Hotels', 'France', '1996-09-26', 70.0, 1.85, ''),
    ('Laurent', 'Pichon', 'Arkéa-B&B Hotels', 'France', '1986-05-19', 72.0, 1.85, ''),
    ('Alan', 'Riou', 'Arkéa-B&B Hotels', 'France', '1997-04-02', 65.0, 1.80, ''),
    ('Guillaume', 'Martin', 'Cofidis', 'France', '1993-06-09', 62.0, 1.78, ''),
    ('Victor', 'Lafay', 'Cofidis', 'France', '1996-01-17', 65.0, 1.80, ''),
    ('Axel', 'Zingle', 'Cofidis', 'France', '1998-12-18', 68.0, 1.85, ''),
    ('Piet', 'Allegaert', 'Cofidis', 'Belgium', '1995-01-20', 75.0, 1.88, ''),
    ('Alexis', 'Renard', 'Cofidis', 'France', '1999-06-01', 70.0, 1.85, ''),
    ('Axel', 'Laurance', 'Cofidis', 'France', '2001-04-13', 70.0, 1.85, ''),
    ('Anthony', 'Perez', 'Cofidis', 'France', '1991-04-22', 68.0, 1.83, ''),
    ('Simon', 'Geschke', 'Cofidis', 'Germany', '1986-03-23', 68.0, 1.85, ''),
    ('Benoît', 'Cosnefroy', 'Decathlon AG2R La Mondiale Team', 'France', '1995-10-17', 65.0, 1.78, ''),
    ('Oliver', 'Naesen', 'Decathlon AG2R La Mondiale Team', 'Belgium', '1990-09-16', 75.0, 1.88, ''),
    ('Greg', 'Van Avermaet', 'Decathlon AG2R La Mondiale Team', 'Belgium', '1985-05-17', 74.0, 1.81, ''),
    ('Felix', 'Gall', 'Decathlon AG2R La Mondiale Team', 'Austria', '1998-02-27', 62.0, 1.78, ''),
    ('Dorian', 'Godon', 'Decathlon AG2R La Mondiale Team', 'France', '1996-05-25', 70.0, 1.85, ''),
    ('Paul', 'Lapeira', 'Decathlon AG2R La Mondiale Team', 'France', '2000-05-28', 68.0, 1.85, ''),
    ('Valentin', 'Paret Peintre', 'Decathlon AG2R La Mondiale Team', 'France', '1999-01-14', 65.0, 1.80, ''),
    ('Nans', 'Peters', 'Decathlon AG2R La Mondiale Team', 'France', '1994-03-12', 68.0, 1.85, ''),
    ('Romain', 'Bardet', 'Team dsm-firmenich PostNL', 'France', '1990-11-09', 62.0, 1.78, ''),
    ('John', 'Degenkolb', 'Team dsm-firmenich PostNL', 'Germany', '1989-01-07', 78.0, 1.81, ''),
    ('Nils', 'Eekhoff', 'Team dsm-firmenich PostNL', 'Netherlands', '1998-01-23', 75.0, 1.88, ''),
    ('Romain', 'Combaud', 'Team dsm-firmenich PostNL', 'France', '1991-04-01', 70.0, 1.85, ''),
    ('Fabio', 'Jakobsen', 'Team dsm-firmenich PostNL', 'Netherlands', '1996-08-31', 78.0, 1.88, ''),
    ('Julien', 'Simon', 'Team dsm-firmenich PostNL', 'France', '1985-10-04', 68.0, 1.83, ''),
    ('Martijn', 'Tusveld', 'Team dsm-firmenich PostNL', 'Netherlands', '1993-09-09', 70.0, 1.85, ''),
    ('Kevin', 'Vermeulen', 'Team dsm-firmenich PostNL', 'Netherlands', '1996-06-15', 75.0, 1.88, ''),
    ('Alexey', 'Lutsenko', 'Astana Qazaqstan Team', 'Kazakhstan', '1992-09-07', 70.0, 1.85, ''),
    ('Mark', 'Cavendish', 'Astana Qazaqstan Team', 'United Kingdom', '1985-05-21', 70.0, 1.75, ''),
    ('Cees', 'Bol', 'Astana Qazaqstan Team', 'Netherlands', '1995-07-25', 78.0, 1.90, ''),
    ('Gianni', 'Moscon', 'Astana Qazaqstan Team', 'Italy', '1994-04-20', 75.0, 1.88, ''),
    ('Harold', 'Tejada', 'Astana Qazaqstan Team', 'Colombia', '1997-04-27', 62.0, 1.78, ''),
    ('Simone', 'Velasco', 'Astana Qazaqstan Team', 'Italy', '1995-12-02', 68.0, 1.85, ''),
    ('Christian', 'Scaroni', 'Astana Qazaqstan Team', 'Italy', '1997-10-16', 68.0, 1.85, ''),
    ('Igor', 'Chzhan', 'Astana Qazaqstan Team', 'Kazakhstan', '1996-03-28', 70.0, 1.85, ''),
    ('Alexander', 'Kristoff', 'Uno-X Mobility', 'Norway', '1987-07-05', 78.0, 1.81, ''),
    ('Tobias', 'Halland Johannessen', 'Uno-X Mobility', 'Norway', '1999-08-07', 68.0, 1.85, ''),
    ('Jonas', 'Abrahamsen', 'Uno-X Mobility', 'Norway', '1995-09-20', 75.0, 1.88, ''),
    ('Anthon', 'Charmig', 'Uno-X Mobility', 'Denmark', '1998-03-25', 68.0, 1.85, ''),
    ('Rasmus', 'Tiller', 'Uno-X Mobility', 'Norway', '1996-07-28', 70.0, 1.85, ''),
    ('Søren', 'Wærenskjold', 'Uno-X Mobility', 'Norway', '2000-03-12', 75.0, 1.88, ''),
    ('Magnus', 'Cort Nielsen', 'Uno-X Mobility', 'Denmark', '1993-01-16', 72.0, 1.85, ''),
    ('Erik', 'Nordsæter Resell', 'Uno-X Mobility', 'Norway', '1996-09-15', 70.0, 1.85, ''),
    ('Biniam', 'Girmay', 'Intermarché-Wanty', 'Belgium', '2000-04-02', 70.0, 1.85, ''),
    ('Louis', 'Meintjes', 'Intermarché-Wanty', 'South Africa', '1992-02-21', 58.0, 1.70, ''),
    ('Mike', 'Teunissen', 'Intermarché-Wanty', 'Netherlands', '1992-08-25', 75.0, 1.88, ''),
    ('Louis', 'Barré', 'Intermarché-Wanty', 'France', '2000-07-06', 70.0, 1.85, ''),
    ('Vito', 'Braet', 'Intermarché-Wanty', 'Belgium', '1995-10-18', 70.0, 1.85, ''),
    ('Hugo', 'Page', 'Intermarché-Wanty', 'France', '2001-07-24', 65.0, 1.80, ''),
    ('Laurenz', 'Rex', 'Intermarché-Wanty', 'Belgium', '1999-12-15', 70.0, 1.85, ''),
    ('Jonas', 'Rutsch', 'Intermarché-Wanty', 'Germany', '1998-01-09', 75.0, 1.88, ''),
    ('Georg', 'Zimmermann', 'Intermarché-Wanty', 'Germany', '1997-10-11', 68.0, 1.85, ''),
    ('Roel', 'Van Sintmaartensdijk', 'Intermarché-Wanty', 'Netherlands', '2000-03-08', 75.0, 1.88, ''),
    ('Fred', 'Wright', 'Bahrain Victorious', 'United Kingdom', '1999-06-13', 70.0, 1.85, ''),
    ('Robert', 'Stannard', 'Bahrain Victorious', 'Australia', '1998-09-16', 70.0, 1.85, ''),
    ('Benjamin', 'Thomas', 'Cofidis', 'France', '1995-09-12', 70.0, 1.88, ''),
    ('Jesús', 'Herrada', 'Cofidis', 'Spain', '1990-07-26', 68.0, 1.80, ''),
    ('Ilan', 'Van Wilder', 'Soudal Quick-Step', 'Belgium', '2000-05-14', 65.0, 1.80, ''),
    ('Valentin', 'Paret Peintre', 'Soudal Quick-Step', 'France', '1999-01-14', 65.0, 1.80, ''),
    ('Matteo', 'Jorgenson', 'Team Visma-Lease a Bike', 'United States', '2000-07-01', 68.0, 1.85, ''),
    ('Simon', 'Yates', 'Team Visma-Lease a Bike', 'United Kingdom', '1992-08-07', 58.0, 1.72, ''),
    ('Marijn', 'Van den Berg', 'EF Education-EasyPost', 'Netherlands', '1999-07-19', 75.0, 1.88, ''),
    ('Michael', 'Valgren', 'EF Education-EasyPost', 'Denmark', '1992-02-07', 70.0, 1.85, ''),
    ('Romain', 'Grégoire', 'Groupama-FDJ', 'France', '2003-03-24', 65.0, 1.80, ''),
    ('Olivier', 'Le Gac', 'Groupama-FDJ', 'France', '1993-08-27', 70.0, 1.85, ''),
    ('Miles', 'Scotson', 'Groupama-FDJ', 'France', '1994-01-18', 70.0, 1.88, ''),
    ('Jasper', 'Stuyven', 'Lidl-Trek', 'Belgium', '1992-04-17', 75.0, 1.88, ''),
    ('Alex', 'Kirsch', 'Lidl-Trek', 'Luxembourg', '1992-06-12', 70.0, 1.85, ''),
    ('Juan', 'Pedro López', 'Lidl-Trek', 'Spain', '1997-07-31', 58.0, 1.70, ''),
    ('Daan', 'Hoole', 'Lidl-Trek', 'Netherlands', '1999-02-22', 70.0, 1.85, ''),
    ('Antwan', 'Tolhoek', 'Lidl-Trek', 'Netherlands', '1994-04-29', 65.0, 1.80, ''),
    ('Sam', 'Oomen', 'Lidl-Trek', 'Netherlands', '1995-08-15', 65.0, 1.80, ''),
    ('Bauke', 'Mollema', 'Lidl-Trek', 'Netherlands', '1986-11-26', 68.0, 1.83, ''),
    ('Maximilian', 'Schachmann', 'Soudal Quick-Step', 'Germany', '1994-01-09', 70.0, 1.85, ''),
    ('Rémi', 'Cavagna', 'Soudal Quick-Step', 'France', '1995-06-10', 75.0, 1.93, '')
) AS r(first_name, last_name, team_name, nationality, date_of_birth, weight_kg, height_m, photo_url)
INNER JOIN teams_pro tp ON tp.name = r.team_name
ON CONFLICT DO NOTHING;

-- Verify the import
SELECT COUNT(*) as total_riders FROM riders;
SELECT 
  r.id,
  r.first_name,
  r.last_name,
  r.date_of_birth,
  tp.name as team_name,
  r.nationality,
  r.weight_kg,
  r.height_m
FROM riders r
LEFT JOIN teams_pro tp ON r.team_pro_id = tp.id
ORDER BY r.last_name, r.first_name
LIMIT 20;
